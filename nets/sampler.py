...
class SubSampler (FullSampler):
    ...
    def __call__(self, feats, confs, aflow):
        feat1, conf1 = feats[0], (confs[0] if confs else None)  #feat1 and feat2 dimension Batch,D(128), H, W
        ...
        B, D, Hq, Wq = feat1.shape  #q for image 1, d for image 2
        B, D, Hd, Wd = feat2.shape
        
        # compute gt
        if self.perimage or self.sub_q != self.sub_d:
            # compute ground-truth by comparing pixel indices
            f = feats[0][0:1,0] if self.perimage else feats[0][:,0] #feats[0]后面[:,0]第一个冒号对应batch, 第二个0对应descriptor第一个dimension
            idxs = torch.arange(f.numel(), dtype=torch.int64, device=feat1.device).view(f.shape) #f.shape may be (batch,h,w), idx shape is also (batch,h,w)
            idxs1 = idxs[:, slq, slq].reshape(-1,Hq*Wq)  #idxs1 dimension (batch,(Hq*Wq))
            idxs2 = idxs[:, sld, sld].reshape(-1,Hd*Wd)  #idxs2 dimension (batch,(Hd*Wd))
            if self.perimage:
                gt = (idxs1[0].view(-1,1) == idxs2[0].view(1,-1)) #gt的dimension是(Hq*Wq)*(Hd*Wd)(二维). 如果图一的a像素和图二的b像素对应那么gt的(a,b)为1,否则为0
        ...
        queries  =  feat1.reshape(B,D,-1) # B x D x (Hq x Wq), D=128
        ...
        if self.perimage:
            ...
            #!!!!!这个其实就是点乘求第二张图片对于第一张图片的所有patches的cosine相似度。每一行代表第一张图片一个patch，
            #每一列代表第二张图片的一个patch，第一行第三列就是第二张图片第三个patch对于第一张图片第一个patch的cosine相似度
        else:
